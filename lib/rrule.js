...
var dateutil = {
    ...
    getYearDay: function(date) {
        var timeZone = dateutil.getTimeZone(date);
        var dateNoTime =
            dateutil.createDate(date.getFullYear(), date.getMonth(), date.getDate(), timeZone);
        var dateStartYear = dateutil.createDate(date.getFullYear(), 0, 1, timeZone);
        return Math.ceil((dateNoTime - dateStartYear) / dateutil.ONE_DAY) + 1;
    },
     ...
     */
    fromOrdinal: function(ordinal, timeZone) {
        var millisecsFromBase = ordinal * dateutil.ONE_DAY;
        var value = dateutil.ORDINAL_BASE.getTime()
            - dateutil.tzOffset(dateutil.ORDINAL_BASE)
            +  millisecsFromBase
            + dateutil.tzOffset(dateutil.createDate(millisecsFromBase, timeZone));
        return dateutil.createDate(value, timeZone);
    },

    /**
     * @see: <http://docs.python.org/library/calendar.html#calendar.monthrange>
     */
    monthRange: function(year, month, timeZone) {
        var date = dateutil.createDate(year, month, 1, timeZone);
    ...
    combine: function(date, time) {
        time = time || date;
        // TODO(aramk) Not sure why, but Time also has a timezone argument associated with it.
        // We should keep it in Date or Time, but not both.
        var timeZone = dateutil.getTimeZone(date) || time.getTimeZone();
        return dateutil.createDate(date.getFullYear(), date.getMonth(), date.getDate(),
            time.getHours(), time.getMinutes(), time.getSeconds(), timeZone);
    },

    clone: function(date) {
        var timeZone = dateutil.getTimeZone(date);
        var dolly = dateutil.createDate(date.getTime(), timeZone);
    ...
    timeToUntilString: function(time, timeZone) {
        var date = dateutil.createDate(time, timeZone);
        var comp, comps = [
            date.getUTCFullYear(),
            date.getUTCMonth() + 1,
            date.getUTCDate(),
    ...
    /**
     * @params {...*} args - Any arguments which Date supports. The last argument may optionally
     *     be a String in this format: http://momentjs.com/timezone/docs. e.g. "America/New_York".
     * @return {Date} If a time zone was provided, the Date class set by {@link RRule.setDateClass}
     *     will be used. Otherwise, a native Date is returned in the local time zone.
     */
    createDate: function() {
        var args = Array.prototype.slice.call(arguments);
        var lastArg = args[args.length - 1];
        var DateClass = this.Date;
        if (DateClass === Date) {
            // Remove the time zone argument since it's not accepted by Date and will fail.
            if (args.length > 1 &&
                (lastArg == null || typeof lastArg === 'string')) {
                args.pop();
            }
            if (args.length === 1 && args[0] == null) {
                // Date does not accept undefined being passed as the only arg to construct the
                // current local time.
                args = [];
            }
            return applyToConstructor(Date, args);
        } else {
            return applyToConstructor(DateClass, args);
        }
    },

    getTimeZone: function(date) {
        return date && date.getTimeZone ? date.getTimeZone() : undefined;
    }

};

dateutil.Time = function(hour, minute, second, millisecond, timezone) {
    this.hour = hour;
    this.minute = minute;
    this.second = second;
...
function applyToConstructor(constructor, argArray) {
    argArray = argArray || [];
    if (argArray != null && argArray.length == null) {
...
var RRule = function(options, noCache) {
    ...
    if (!opts.dtstart) {
        ...
        opts.dtstart = dateutil.createDate(undefined, opts.timezone);
        ...
        for (i = 0; i < opts.byhour.length; i++) {
            ...
            for (var j = 0; j < opts.byminute.length; j++) {
                ...
                for (var k = 0; k < opts.bysecond.length; k++) {
                    ...
                    // tzinfo=self._tzinfo))
                    this.timeset.push(new dateutil.Time(hour, minute, second, millisecondModulo, opts.timezone));
                }
            }
        }
...
RRule.setDateClass = function(DateClass) {
...
RRule.prototype = {
    ...
    all: function(iterator) {
        var args = {rrule: this};
        if (iterator) {
            return this._iter(new CallbackIterResult('all', args, iterator));
        } else {
            var result = this._cacheGet('all');
            if (result === false) {
                result = this._iter(new IterResult('all', args, RRule));
            ...
            if (plb(bysetpos) && plb(timeset)) {
                ...
                for (i, j = 0; j < bysetpos.length; j++) {
                    ...
                    try {
                        ...
                        var date = dateutil.fromOrdinal(ii.yearordinal + i, timezone);
...
RRule.parseString = function(rfcString) {
    ...
    // Parse timezone first since it's used for all dates.
    ...
    for (i = 0; i < attrs.length; i++) {
        ...
        switch (key) {
            ...
            case 'BYDAY': // => byweekday
                ...
                break;
            case 'DTSTART':
                options.dtstart = dateutil.untilStringToDate(value, timezone);
                break;
            case 'UNTIL':
                options.until = dateutil.untilStringToDate(value, timezone);
                break;
            case 'BYEASTER':
...
Iterinfo.prototype.rebuild = function(year, month) {
    ...
    var rr = this.rrule;
    var timeZone = rr.options.timezone;

    if (year != this.lastyear) {

        this.yearlen = dateutil.isLeapYear(year) ? 366 : 365;
        this.nextyearlen = dateutil.isLeapYear(year + 1) ? 366 : 365;
        var firstyday = dateutil.createDate(year, 0, 1, timeZone);

        this.yearordinal = dateutil.toOrdinal(firstyday);
        this.yearweekday = dateutil.getWeekday(firstyday);

        var wday = dateutil.getWeekday(dateutil.createDate(year, 0, 1, timeZone));
            ...
            if (no1wkst) {
                ...
                if (!contains(rr.options.byweekno, -1)) {
                    var lyearweekday = dateutil.getWeekday(
                        dateutil.createDate(year - 1, 0, 1, timeZone));
                    var lno1wkst = pymod(
...
Iterinfo.prototype.wdayset = function(year, month, day) {
    var timeZone = this.rrule.options.timezone;
    // We need to handle cross-year weeks here.
    var set = repeat(null, this.yearlen + 7);
    var i = dateutil.toOrdinal(
        dateutil.createDate(year, month - 1, day, timeZone)) - this.yearordinal;
    var start = i;
    for (var j = 0; j < 7; j++) {
        set[i] = i;
        ++i;
...
Iterinfo.prototype.htimeset = function(hour, minute, second, millisecond) {
    ...
    for (var i = 0; i < rr.options.byminute.length; i++) {
        ...
        for (var j = 0; j < rr.options.bysecond.length; j++) {
            ...
            set.push(new dateutil.Time(hour, minute, second, millisecond, rr.options.timezone));
        }
    }
    dateutil.sort(set);
    return set;
...
Iterinfo.prototype.stimeset = function(hour, minute, second, millisecond) {
    return [new dateutil.Time(hour, minute, second, millisecond, this.rrule.options.timezone)];
};
...
IterResult.prototype = {
    ...
    init: function(method, args) {
        ...
        var timezone = args.rrule.options.timezone;
        ...
        if (method == 'between') {
            this.maxDate = args.inc
                ? before
                : dateutil.createDate(before.getTime() - 1, timezone);
            this.minDate = args.inc
                ? after
                : dateutil.createDate(after.getTime() + 1, timezone);
        } else if (method == 'before') {
            this.maxDate = args.inc ? args.dt :
                dateutil.createDate(args.dt.getTime() - 1, timezone);
        } else if (method == 'after') {
            this.minDate = args.inc ? args.dt :
                dateutil.createDate(args.dt.getTime() + 1, timezone);
        }
    },
     ...
     */
    add: function(date) {
        var timezone = this.args.rrule.options.timezone;
        this._result.push(dateutil.createDate(date.valueOf(), timezone));
        return true;
    },
